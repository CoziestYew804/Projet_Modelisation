...
/**
 *
 * RESULTAT : un vecteur al�atoire x tel que | x - germe | < rayon
 *
...
//static double produitScalaire (const Vecteur2D & u1, const Vecteur2D & u2) {return u1*u2;}
    ...
    const Vecteur2D rotationDirecteQuartDeTour () const
    { return Vecteur2D(-y, x); }


/**
 * cr�e le vecteur (y,x) � partir du vecteur (x,y)
 * */
 ...
 * TACHE : calcule les valeurs arrondies des coordonn�es de *this
 *
 * */
    template<class T>
    void arrondit (T &xArrondi, T &yArrondi) const
    {
        xArrondi = (T) x;
        yArrondi = (T) y;
    }
 ...
 * */
    inline static double normeSup (const Vecteur2D &u)
...
inline const Vecteur2D random (const Vecteur2D &germe, const double &rayon)
{ return Vecteur2D::random(germe, rayon); }
 ...
 *
 * */
/*static*/ inline double Vecteur2D::det (const Vecteur2D &u1, const Vecteur2D &u2)
 ...
 *
 * Donne des r�sultats inexploitables si v1 = 0 ou si v2 = 0
 *
 * */
