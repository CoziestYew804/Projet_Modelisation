...
inline int signe (const double &a)
{ return a >= 0 ? 1 : -1; }

inline const Vecteur2D abs (const Vecteur2D &v)
...
repr�sente la transformation v |---> M*v + B
*/
...
*/
    ...
    const TransfoAffine2D inverse () const
    {
        Matrice22 M1 = M.inverse();
        return TransfoAffine2D(M1, -M1 * B);
    }

/**
 * HYPOTHESES :
 * Notons
 * On suppose que {coinBG1, coinHD1} repr�sente la diagonale d'un rectangle R1. ce sont respectivement les coins bas gauche et haut droit de R1
 * R1 est donn� dans un certain rep�re Rep1 (O1,i1,j1),
 * R1 n'est pas d�g�n�r�
 * On suppose que {coinBG2, coinHD2} repr�sente la diagonale d'un rectangle R2. ce sont respectivement les coins bas gauche et haut droit de R2
 * R2 est donn� dans un certain rep�re Rep2 (O2,i2,j2)
 * R2 n'est pas d�g�n�r�
 *
 * On suppose qu'on peut transformer  R1 en  R2 sans rotation, c'est-�-dire uniquement par translation et/ou �tirement sur l'axe x ou sur l'axe y
 *
 * Autrement dit, on suppose que i1 et i2 sont colin�aires et j1 et j2 sont aussi colin�aires
 *
 *
 * TACHE :
 *
 * Le but est de calculer le changement de rep�re t qui transforme R1 en R2
  *
 * construit donc la transfo affine 2D t tel que :
 * 1) t(R1) ne soit pas d�form� par rapport � R1
